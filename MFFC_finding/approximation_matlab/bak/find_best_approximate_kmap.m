function [bestApproximateTruthTable,approximate_error] = find_best_approximate_kmap(kmap)



row_id_all_0s_vec = [];
row_id_all_1s_vec = [];
row_id_neither_all_0s1s_vec = [];
truthTable_reduced = [];
for i=1:1:size(kmap,1)
    row_vec = kmap(i,:);
    if(get_is_all_0s(row_vec)==1)
        row_id_all_0s_vec = [row_id_all_0s_vec,i];
    elseif(get_is_all_1s(row_vec)==1)
        row_id_all_1s_vec = [row_id_all_1s_vec,i];
    else
        row_id_neither_all_0s1s_vec = [row_id_neither_all_0s1s_vec, i];
        truthTable_reduced = [truthTable_reduced;row_vec];
    end
end

truthTable_reduced_column_count = size(truthTable_reduced,2);
maj_vec_original = zeros(1,truthTable_reduced_column_count);

for i=1:1:truthTable_reduced_column_count
    count_of_1s = sum(truthTable_reduced(:,i));
    count_of_0s = sum(truthTable_reduced(:,i)==0);
...
for dec_value=1:1:2^dont_care_count
	...
	bin_vec = [zeros(1,dont_care_count-length(vec1)),vec1];
...
for i=1:1:solution_choice_total_number
    ...
    truth_table_original = truthTable_reduced;
    sol_choice_obj = init_solution_choice(sol_choice_obj, maj_vec, truth_table_original);
    sol_choice_obj = set_cost_vec_matrix(sol_choice_obj);
    sol_choice_obj = set_best_approximate_truthTable(sol_choice_obj);
...
truth_table_approximate = kmap;
for i=1:1:length(row_id_neither_all_0s1s_vec)
    row_id = row_id_neither_all_0s1s_vec(i);
    truth_table_approximate(row_id,:) = sol_choice_optimal.kmap_modified(i,:);
end

bestApproximateTruthTable = truth_table_approximate;

approximate_error = sum(sum(xor(kmap,truth_table_approximate)));
