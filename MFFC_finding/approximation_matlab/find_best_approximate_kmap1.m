function [approximate_kmap_obj,approximate_error_count] = find_best_approximate_kmap1(kmap_obj, enable_fast_mode, dont_care_upper_bound)

kmap = kmap_obj.kmap;

row_id_all_0s_vec = [];
row_id_not_all_0s_vec = [];
kmap_reduced = [];
for i=1:1:size(kmap,1)
    ...
    if(get_is_all_0s(row_vec)==1)
        ...
        row_id_not_all_0s_vec = [row_id_not_all_0s_vec, i];
...
for i=1:1:solution_choice_total_number
    ...
    sol_choice_obj = set_cost_vec_matrix1(sol_choice_obj);
    sol_choice_obj = set_best_approximate_kmap1(sol_choice_obj);
    %i
    %sol_choice_obj
...
for i=1:1:length(row_id_not_all_0s_vec)
    row_id = row_id_not_all_0s_vec(i);
